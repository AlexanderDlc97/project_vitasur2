    ...
    <div class="card border-4 borde-top-primary shadow-sm h-100" style="border-radius: 20px; min-height: 500px" data-aos="fade-up" data-aos-anchor-placement="top-bottom">
        <div class="card-header bg-transparent">
            <div class="row justify-content-between">
                <div class="col-9 col-md-3 col-xl-2 mb-2 mb-lg-0 order-1 order-md-1 order-lg-1 order-xl-1">
                    <a href="{{ route('admin-medicos.create') }}" class="btn btn-primary btn-sm text-uppercase text-white w-100" style="border-radius: 20px">
        ...
        <div class="card-body">
            <div class="mb-2 col-12 col-md-6">
                <span class="text-uppercase">Total de registros encontrados: <span class="fw-bold">{{$admin_medicos->count()}}</span></span>
            </div>
            <table id="display" class="table table-hover table-sm" cellspacing="0" style="width:100%">
                <thead class="bg-dark text-white border-0">
                    <tr>
                        ...
                        <th class="h6 small text-center text-uppercase fw-bold">Profesión</th>
                ...
                <tbody>
                    @php
                        $contador = 1;
                    @endphp
                    @foreach ($admin_medicos as $admin_medico)
                        <tr>
                            ...
                            <td class="text-center align-middle">
                                ...
                                @else
                                    <img src="/images/personas/{{$admin_medico->imagen}}" class="img-fluid" style="width: 40px; height: 40px; objet-fit:cover" alt="">
                                @endif
                            </td>
                            <td class="fw-normal text-center align-middle text-md-center">{{$admin_medico->name.' '.$admin_medico->surnames}}</td>
                            <td class="fw-normal text-center align-middle text-md-center">{{$admin_medico->identificacion.': '.$admin_medico->nro_identificacion}}</td>
                            <td class="fw-normal text-center align-middle text-md-center">{{$admin_medico->nro_contacto}}</td>
                            <td class="fw-normal text-center align-middle text-md-center">{{$admin_medico->medico->profesion->name}}</td>
                            <td class="fw-normal align-middle small text-md-center">
                                <form method="POST" action="/admin-medicos/estado/{{ $admin_medico->slug }}" class="form-update">
                                    @csrf
                                    @method('PUT')
                            ...
                            <td class="text-center align-middle">                                        
                                <div class="text-start text-md-center">
                                    <div class="dropstart">
                                        ...
                                        <ul class="dropdown-menu">                                                
                                            <li class="dropdown-item">
                                                <button class="bg-transparent border-0 px-0 mx-0" data-bs-toggle="modal" data-bs-target="#showmedico{{ $admin_medico->slug }}"><i class="bi bi-eye-fill me-2"></i>Ver detalles</button>
                                            </li>      
                                            <li class="dropdown-item">
                                                <a href="{{ route('admin-medicos.edit',$admin_medico->slug) }}" class="text-decoration-none" style="color: #000;"><i class="bi bi-pencil-square me-2"></i>Editar</a>
                                            </li>  
                                            <li class="dropdown-item">
                                                <form method="POST" action="{{ route('admin-medicos.destroy',$admin_medico->slug) }}" class="form-delete">
                                                    @csrf
                                                    @method('DELETE')
...
{{-- Fin contenido --}}
    @foreach($admin_medicos as $admin_medico)
        @include('ADMINISTRADOR.PRINCIPAL.medicos.show')
    @endforeach
@endsection
@section('js')
