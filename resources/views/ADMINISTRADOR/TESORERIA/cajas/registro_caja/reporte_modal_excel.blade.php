    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('admin-caja.resultadosEXCEL') }}" method="POST">
                    @csrf
                    <div class="my-3" id="">
                        <label for="">Filtrar</label>
                        <input type="date" name="fecha_ini"  id="fecha_ini" class="form-control form-control-sm">
                    </div>
        
                    <button type="submit" target=_blank  class="btn btn-dark w-100 mt-3">Generar Reporte</button>
                </form>
                {{-- <a href="{{ route('print-salidas.excel') }}" class="border border-secondary text-secondary btn w-100 mt-3">Descargar Todo</a> --}}
            </div>
        </div>
    </div>
</div>