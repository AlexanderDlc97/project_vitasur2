...
class PagoExportFecha implements FromView, WithColumnWidths, WithCustomStartCell, WithTitle
{
    /**
    ...
    public function view(): View
    {
        ...
        if(Auth::user()->role_id == '1'){
            if($this->fecha_asignada_value == 'fecha_asignada' || $this->fecha_asignada_value == ''){
                ...
                $pagos = Pago::whereBetween('created_at', [$this->inicio,$this->fin])->get();
            }
        }if(Auth::user()->role_id == '4'){
            if($this->fecha_asignada_value == 'fecha_asignada'){
                $pagos = Pago::where('sede_id',Auth::user()->persona->sede_id)->get();
            }else{
                $pagos = Pago::where('sede_id',Auth::user()->persona->sede_id)->whereBetween('created_at', [$this->inicio,$this->fin])->get();
            }
        }
        return view('ADMINISTRADOR.REPORTES.pago.xlsx_lista_pago', compact('pagos'));
    }
